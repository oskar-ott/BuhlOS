...
//
// PURE — no I/O, no globals (the only require is the pure brand constant). Unit-tested from src so the gates
...
const { FIELD_APP_NAME: APP } = require('./brand');
...
function inviteEmail(ctx) {
  ...
  const note = ctx.optionalNote
    ...
    : '';
  const bodyHtml = `<p style="font-size:15px;line-height:1.5;color:${INK};margin:0 0 14px">${esc(ctx.adminName)} at <b>${esc(ctx.companyName)}</b> has added you as ${article(role)} ${esc(role)}. ${APP} is the app you'll use on your phone every day.</p>${note}
...
<td style="padding:12px 0 12px 8px;font-size:13px;color:${INK};vertical-align:top"><b style="color:${BRAND_NAVY}">Jobs</b><br>Site info, photos.</td>
  ...
  return {
    subject: `You're invited to ${APP} — ${ctx.companyName}`,
    html: shell({
      heading: `G'day ${esc(ctx.firstName)}.<br>You've been invited to ${APP}.`,
      bodyHtml,
      ctaLabel: `Set up ${APP}`,
      ctaUrl: ctx.ctaUrl,
    ...
    text: plainShell({
      ...
      lines: [
        ...
        `${APP} is the app you'll use on your phone every day to:`,
        '',
...
function resendEmail(ctx) {
  const bodyHtml = `<p style="font-size:15px;line-height:1.5;color:${INK};margin:0 0 14px">${esc(ctx.adminName)} sent this last week. Take 2 minutes to set up ${APP} so you can log your hours. <b>This is a fresh link — any earlier one no longer works.</b></p>`;
  return {
    subject: `Reminder: set up ${APP} — ${ctx.companyName}`,
    html: shell({
    ...
    text: plainShell({
      ...
      lines: [
        `${ctx.adminName} sent this last week. Take 2 minutes to set up ${APP} so you can log your hours.`,
...
function expiredReplacementEmail(ctx) {
  ...
  return {
    subject: `New invite for ${APP} — ${ctx.companyName}`,
    html: shell({
...
function acceptedNotificationEmail(ctx) {
  ...
  return {
    subject: `${ctx.firstName} ${ctx.lastName} is in ${APP}`,
...
<h1 style="font-size:20px;color:${BRAND_NAVY};margin:0 0 12px;font-weight:700">${esc(ctx.firstName)} ${esc(ctx.lastName)} is in ${APP}.</h1>
<p style="font-size:15px;line-height:1.5;color:${INK};margin:0 0 16px">${esc(ctx.firstName)} set up ${APP} at ${esc(ctx.timeText)}.${job}</p>
<a href="${esc(ctx.buhlosUrl)}" style="font-size:14px;color:${BRAND_NAVY};text-decoration:underline">View in BuhlOS &rarr;</a>
</td></tr></table></body></html>`,
    text: `${ctx.firstName} ${ctx.lastName} is in ${APP}.\n\n${ctx.firstName} set up ${APP} at ${ctx.timeText}.${ctx.jobText ? ` He's on ${ctx.jobText} today.` : ''}\n\nView in BuhlOS: ${ctx.buhlosUrl}`,
  };
}
