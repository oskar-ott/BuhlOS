// Process-local diagnostics for the FIELD/Phil evidence-metadata READ OVERLAY
// (supabase_read_phil_evidence). Separate counter from the admin evidence overlay,
// the task-read modules, and the evidence parity PROBE, so the field evidence
// cutover is observed independently (the J10/J11 per-audience-module pattern).
//
...
// field evidence reads served by THIS warm instance. No PII, no evidence content,
// no ids/URLs/note bodies — only counts, booleans, a sanitised reason, and latency.

function freshState() {
  return {
    resetAt: new Date().toISOString(),
    totalReads: 0, // field evidence reads served (flag on)
...
/** Record one field evidence read. Never throws. */
function recordPhilEvidenceRead(diag) {
...
function getPhilEvidenceReadDiagnostics() {
  return { ...state, lastDiag: state.lastDiag ? { ...state.lastDiag } : null };
}
function resetPhilEvidenceReadDiagnostics() {
  state = freshState();
}

module.exports = { recordPhilEvidenceRead, getPhilEvidenceReadDiagnostics, resetPhilEvidenceReadDiagnostics };
