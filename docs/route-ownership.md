...
| `/expenses` | `src/app/(admin)/expenses/page.tsx` | **Reimbursement queue** (#536). Cross-worker review of field-submitted receipts (receipt photo + amount + category): submitted → approved → reimbursed (or declined). Admin-tier gated (matches the `/api/expenses` admin list gate). Money is integer cents; payout runs through payroll/Xero (#249), not here (payroll-boundary ADR #609). |
...
| `/owner` | `src/app/(admin)/owner/page.tsx` | **Owner Console — platform control** (`docs/owner-console.md`). The product/platform-owner control surface: app health, usage, feature-flag visibility (read-only), product problems, audit trail, surface coverage, next actions. **OWNER-ONLY** — role `owner` OR the `OWNER_EMAILS` allowlist; narrower than the admin tier. Coarse-gated to admin in `middleware.ts`, then authoritatively gated by `GET /api/owner` (a non-owner gets `403` → the page calls `notFound()`). Read-only first slice — no flag toggles (no safe override write-path exists yet). Distinct from `/reports` (business owner *numbers*); this is platform/product *observability*. Not in the shared sidebar — reached via the owner login landing + direct URL. |
...
| `/v2/jobs/[jobId]` (+ `/evidence`, `/photos`, `/snags`, `/itps`, `/documents`, `/builder`, `/observations`, `/material-requests`, `/history`, `/plans`, `/diary`) | `src/app/v2/jobs/[jobId]/**` | Live admin job hub + review sections. #210 added `/diary` (per-job site diary — the office's contemporaneous daily record / delay-claim evidence; admin writes, LH reads, via `api/diary.js`; Phil write deferred). `/builder` is the **Job Builder / Editor** (admin only — basics/structure/field-modules/Phil-preview/publish). PR 8 added `/observations` (per-job slice of the cross-job inbox; LH read-only, admin-tier can triage); PR 9 added `/history` (per-job activity feed reading the audit-log via `scope=job`; admin/LH only); PR 11 added `/material-requests` (per-job slice of the procurement inbox; LH read-only, admin-tier can act). The Plans-Phase-1 PR added `/plans` (read-only raster Plan Viewer; admin/LH see **current + superseded**, never archived; gated on the job's `plans` module flag). #242 added `/photos` (read-only "Job Bible" photo gallery — browse every photo on the job: evidence captures + snag + ITP/dwelling photos, date-grouped + filterable; admin/LH; a pure read projection over `/api/evidence` + `/api/photos-catalog`, distinct from `/evidence` which is the review queue). Other sections are admin/LH. Same future move as `/v2/jobs`. |
| `/v2/quotes` | `src/app/v2/quotes/page.tsx` | **The v2 quotes list** (#183, first surface of the Epic 7 rebuild per the #172 MIGRATE-BY-REBUILD ruling — `docs/quoting-legacy-audit.md`). Uses `AdminShell`, gated **admin tier only** (commercial figures; no LH read access, unlike `/v2/jobs`). Reads the NEW `quotes-v2.json` registry — the two legacy draft shells stay in the legacy store and are deliberately unlisted (honest in-page note). Sidebar "Quotes" (Jobs group) links here. Parked on `/v2/quotes` (no `vercel.json` change); same `/admin/quotes` canonical-URL future as `/v2/jobs`. Legacy `/admin/quotes` keeps 307ing to `/command-centre` (§6) until that cutover. |
| `/v2/quotes/[quoteId]` | `src/app/v2/quotes/[quoteId]/page.tsx` | **The v2 quote builder** (#183): sections of line items (kind/description/qty/unit/rate) with live totals from `src/domains/quoting/totals.ts` (the server recomputes the same numbers on save — `api/quotes-v2.js`). Full-document save with an `updatedAt` precondition: a stale save 409s and the builder offers to load the server copy. Admin tier only. Versions/approval/PDF/acceptance/conversion are **dark** here — owned by sibling Epic 7 issues. Same future move as `/v2/quotes`. |
...
| `/expenses` | BuhlOS | `(admin)/expenses` | AdminShell | canonical | admin | sidebar | reimbursement review queue (#536); unauth → 307 `/v2/login` |
...
| `/v2/jobs/[jobId]/scope` | BuhlOS | `v2/jobs/[jobId]/scope` | AdminShell | transitional | **admin** | hub section nav "Scope reconciliation" | read-only scope-vs-quote reconciliation review (#366); confirmed RAG + findings + clause classifications from `scope-reconciliation.json`; authoring stays on `/v2/jobs/[jobId]/job-control`; unauth → 307 `/v2/login` |
| `/v2/jobs/[jobId]/closeout` | BuhlOS | `v2/jobs/[jobId]/closeout` | AdminShell | transitional | **admin** | hub section nav "Closeout", hub closeout-matrix card | closeout matrix (#374) — per-job handover obligations seeded from the recon's `closeout` clauses + electrical defaults, each discharged by typed links to real records (certificate/document/evidence/ITP/as-built) + an admin confirm, with honest N-of-M readiness. Status is RE-DERIVED server-side (links + confirmation + live record ids), so a deleted record reverts a requirement (never orphaned-green). Reads + writes the SPINE `jobs/<jobId>/job-control.json#closeoutRequirements[]` via `POST /api/job-control/closeout/confirm` (HMAC admin-gated, revision precondition) — NOT `closeout.json` (that's #349's numbers freeze). READ-ONLY on completion: never gates/freezes the job's close-out. unauth → 307 `/v2/login` |
| `/api/job-control/test-records` | API | `src/app/api/job-control/test-records/route.ts` | n/a | transitional | field (POST) / admin + assigned worker (GET) | n/a (consumed by the Phil job-detail capture flow + the `/v2/jobs/[jobId]/job-control` `TestRecordsPanel`) | structured electrical TestRecords (#517). **POST** field-gated exactly like `/api/job-control/evidence-link` (401 no session, 403 `!canAccessSurface(role,'phil')`, then the `/api/jobs?id=` assignment gate): writes an immutable, supersede-by-revision record to `jobs/<jobId>/test-records.json` (pass/fail DERIVED server-side via `buildTestRecord`), then mints a companion `test_result` `EvidenceItem` so the EXISTING proof loop flips the requirement to met — extend-never-fork, no `testRecordId` arm on `EvidenceLink`. **GET** `?jobId=` lists records (admin any job; assigned worker their own). `force-dynamic` + `runtime nodejs`. |
...
| `/v2/jobs/[jobId]/diary` | BuhlOS | `v2/jobs/[jobId]/diary` | AdminShell | transitional | admin/LH | hub section nav "Diary" | per-job site diary (#210); date-keyed daily record — happenings/weather/attendance snapshot/delay flag — the office's contemporaneous delay-claim evidence; data via `api/diary.js` (READ admin/LH-on-job, WRITE admin-only; tradie/client 403). Integrity rules enforced + tested: one entry per (job,date), future-date reject, delay-requires-reason, frozen attendance snapshot, append-only amendments. Phil write deferred. Archived job → read-only. unauth → 307 `/v2/login` |
| `/v2/jobs/[jobId]/minutes` | BuhlOS | `v2/jobs/[jobId]/minutes` | AdminShell | transitional | admin/LH | hub section nav "Minutes" (flag-gated) | per-job meeting-minutes register (#217); APPEND-ONLY record of meeting minutes (date/title/attendees/body and/or PDF attachment) with stamped amendments — the original body is never mutated. Data via `api/job-minutes.js` (READ admin/managing-LH, WRITE admin-only). Dark behind the global `minutes_register` flag (404s when off; not in REQUIRED_SOURCES while flag-dark — the `v2/jobs` root is already shell-covered). Attachments are a direct-blob sibling (`jobs/<id>/minutes/<id>.<ext>`), not a Documents-register category. unauth → 307 `/v2/login` |
...
| `/v2/jobs/new` · `/v2/jobs/[jobId]` (+ evidence/photos/snags/itps/documents/observations/material-requests/history) | BuhlOS | AdminShell | `src/app/v2/jobs/[jobId]/**` | transitional | wrong shell | `check-shell-contract`, `middleware.test` |
