...
| --- | --- | --- | --- | --- | --- |
| **Unit** (`npm run test:unit`, `src/**/*.test.ts`) | every PR + push (CI `check`) | no | no | no (mocked Blob) | pure logic + contracts — e.g. `src/middleware.test.ts` (full redirect/landing/role gate), `src/domains/qa/time-entry-attribution.test.ts` (the #77 attribution rule), API handlers against an in-memory Blob |
...
| `phil.spec.ts` · open assigned job | mobile-phil | opens the exact seeded fixture job; `Capture evidence` present; **no** Save/Publish | field | no | Preview Smoke | field job view losing read-only safety |
