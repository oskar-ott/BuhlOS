...
| D55-L9 | ~~`api/_lib/auth.js#canWrite` only recognizes `admin/tradie/leadingHand` role strings.~~ **Resolved at the entry gate by [PR #23](https://github.com/oskar-ott/BuhlOS/pull/23) (2026-05-25, Session 7):** `api/_lib/auth.js` now normalises every tier per [src/lib/auth/roles.ts](../../src/lib/auth/roles.ts) (admin: `admin/boss/owner/manager/office/pm/estimator`; LH: `leadinghand/leading_hand/leading-hand/lh`; field: `tradie/apprentice/labourer/electrician`) and exports the helpers so endpoint-specific role checks can converge. **Still pending:** `api/snags.js` has its own local `ADMIN_ROLES` / `FIELD_ROLES` sets that drop `pm`/`estimator` (admin) and `labourer`/`electrician` (field) — so those roles pass the entry gate but `canRoleTransition` still 403s. Follow-up: import the helpers from `api/_lib/auth.js` and remove the local sets. |
