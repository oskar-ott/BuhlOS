...
- **Long asset names overflow the admin table.** The Asset cell is not truncated; if the asset name exceeds ~30 characters, the Manage button moves off-screen and requires a horizontal scroll. Workaround: keep names short, or use the search box. (BUG-C-005, open.)
