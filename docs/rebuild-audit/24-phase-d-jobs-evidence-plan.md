...
| D-27 | **Vercel Blob read-after-write lag — confirmed in Phase C BUG-C-004 (fixed via `readBlobFresh` in PR #6).** Each Vercel function instance maintains its own `readBlob` cache; writes only invalidate the local instance. Cross-instance staleness window is `BLOB_TTL_MS` (5s). A client that writes EvidenceItem and immediately re-fetches the job's evidence list can hit a *different* warm Vercel instance and read stale data missing the just-written item. | Confirmed (in Phase C; carries over) | Medium | **Binding rules for Phase D's `/api/jobs/[jobId]/evidence/*` endpoints:** (a) every POST/PATCH endpoint MUST return the canonical updated EvidenceItem in the response body — clients consume the response, never refetch immediately; (b) any GET that runs immediately after a mutation in the admin Evidence panel uses `readBlobFresh` (introduced in PR #6's `api/_lib/blob.js`); (c) Playwright tests for Phase D include a "rapid action sequence" test (capture → list → expect new item visible) to catch any re-introduction. |
| D-28 | **Sheet/dialog stays open after action → user double-taps → duplicate history rows** (confirmed in Phase C BUG-C-003, fixed in PR #6). | Confirmed (in Phase C; carries over) | Medium | **Binding rule for Phase D CaptureSheet (and any future Phil sheet):** close the sheet on the first Submit tap; the success banner lands when the async settles. Disabled button alone is insufficient — the sheet itself must close so the user has no path to re-tap. Mirror this in admin Reject modals (close on first submit). Server-side idempotency by `clientCapturedAt + capturedById` (already in plan §11 D-22) is the second line of defence. |
| D-29 | **Mark-good / state-reset pattern from Phase C** (PR #9): worker can flag (one-way), only admin can clear (admin-side reset). Maps to Phase D evidence: worker can capture / re-capture, only admin can review / reject. | Pattern confirmed | Low | Already encoded in Phase D §5.5 status machine + decision §15.0 #6 (LH read-only, admin-only review). PR #9 confirms the precedent is right. The `admin_updated` history kind from Phase C maps to Phase D's `evidence.reviewed` and `evidence.rejected` AuditLog action types — distinct from `evidence.captured` so the audit trail clearly attributes admin action vs worker action. |
...
- [25-phase-d-build-prompts.md](25-phase-d-build-prompts.md) — paste-ready build prompts for D1–D6.
- [26-phase-d-testing-checklist.md](26-phase-d-testing-checklist.md) — per-slice testing + the §A.1 RSC-manifest grep.
- [27-interface-usability-pass.md](27-interface-usability-pass.md) — binding UX rules for both surfaces; cited in every Dx build prompt preflight (§4 Phil, §5 Admin, §6 marker dictionary).
- `phase-c-rollout-runbook.md` (added in PR #9 — phase-c-mark-good branch) — Phase C live state + Phase C/D interaction lessons. After PR #9 merges, this doc becomes the operational reference for the gear loop; risk D-27 / D-28 / D-29 cite its underlying findings.
