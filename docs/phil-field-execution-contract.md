...
2. **`feat/phil-worker-visible-tasks`** (gap #1) — ✅ **SHIPPED** (Path C). Wired `/api/task-toggle` into `PhilJobAreaDetail`: each worker-visible task shows its real state and a Mark done / Undo control (non-optimistic — state only moves on a confirmed server write, so a failed toggle never shows done). Added defence-in-depth API guards (draft/archived job, archived area/group, archived task) and a pure `taskState.ts` helper. No fake progress; binary done/undo in v1 (in_progress displayed, not written); no per-task evidence link (evidence has no `taskId`). See **`docs/phil-worker-visible-tasks.md`**.
