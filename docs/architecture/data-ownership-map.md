...
| Payroll batches | `payroll_batches`, `payroll_batch_items`, `payroll_batch_events` | #893: durable creation-time snapshots (source hours + worker/earnings mappings by immutable Xero ids) — **immutable once locked (DB-trigger-enforced)**; corrections supersede via `supersedes_batch_id`, never mutate; events are append-only. The **single export source** (#895 convergence) that #249 exports from and reconciles against |
| Payroll export ledger | `payroll_batch_timesheet_attempts`, `payroll_batch_timesheet_events` | #249: the draft-timesheet export record — one **immutable** attempt row per Xero POST (trigger-enforced), inserted with its terminal outcome; append-only lifecycle + readback-reconciliation events. Holds the Xero TimesheetID/correlation id + a safe error category, **never** tokens/secrets. Compatibility bridge back to `time_entries` (`exportId`/`exportedAt`) is stamped per-worker only after accepted **and** verified. Gated by `xero_payroll_export` (default off) — DRAFT timesheets only (payroll-boundary ADR #609) |
