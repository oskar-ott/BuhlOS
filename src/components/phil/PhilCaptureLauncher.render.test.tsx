 ...
 * same tap, this is the in-sheet fallback/repeat affordance). Photo/batch
 * interaction is covered by capture-batch.test.ts (submit loop) and
 * philCapture.test.ts (preselection); the real-browser open/close proof lives
 * in the Preview Smoke (phil.spec.ts).
 */
describe("PhilCaptureLauncher", () => {
  ...
  it("opens camera-first: photo tray affordance only", () => {
    const html = renderToString(
