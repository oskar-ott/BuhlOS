...
describe("SplitDaySheet", () => {
  it("renders a total, two job rows and a computed remainder when open", () => {
    ...
    // Total + per-row hours are h/m PAIRS (owner-directed 2026-08-10) — the
    // sheet's decimal boxes were the app's last "4.36 meaning 4h 36m" trap.
    expect(html).toContain("split-total");
    expect(html).toContain("split-total-m");
    expect(html).toContain("split-hours-0");
    expect(html).toContain("split-minutes-0");
  ...
  it("pre-fills total + rows and shows a custom title and error (resubmit mode, #128)", () => {
    ...
    // Seeded total renders as h + m (7.6 → 7h 36m), never a decimal
    // (owner-directed 2026-08-10 — the sheet's decimal boxes are gone).
    expect(html).toContain('value="7"');
    expect(html).toContain('value="36"');
    expect(html).not.toContain('value="7.6"');
    expect(html).not.toContain('step="0.1"');
