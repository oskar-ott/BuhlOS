...
/**
 * The overtime presets in the custom sheet (owner-directed 2026-08-09; first
 * shipped as chips on the standard day 2026-08-07). Workers think "normal
 * day + 1 hour OT", never totals — one worker typed "8.36" meaning 8h 36m,
 * another reads an extra hour as "9 hours", and both pays come out wrong.
 * Each value is DECIMAL HOURS added to STANDARD_DAY_HOURS; the custom sheet
 * renders them as presets (+30m OT / +1h OT / +1½h OT / +2h OT) that FILL
 * the exact-time inputs with the derived total, so nobody does arithmetic.
 */
export const STANDARD_DAY_OT_ADD_ONS = [0.5, 1, 1.5, 2] as const;

/**
 * STANDARD_DAY_HOURS plus an OT add-on, rounded to the store's 2dp
 * (7.6 + 1 → 8.6). The ONE derivation the preset chips and the payload
 * share, so what the worker reads is what the server receives.
 */
