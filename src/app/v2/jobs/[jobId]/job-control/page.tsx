...
/**
 * /v2/jobs/[jobId]/job-control — admin required-proof authoring (#471 follow-up).
 *
 * The click-path that replaces DevTools JSON: an admin classifies a scope clause
 * as field work, ties it to an existing worker task, names the required proof,
 * and runs the shipped L0 reconciliation + L1 compile routes so Phil shows
 * "Capture proof". Server component mirrors the other job sub-routes (documents/
 * itps/snags): gate auth, fetch /api/jobs?id=, render AdminShell + client panel.
 *
 * ADMIN-ONLY — the reconciliation/compile confirm routes require an
 * HMAC-verified admin, so this surface is gated to the admin tier (not LH), and
 * the panel's writes will be rejected server-side for anyone else regardless.
 */
export default async function AdminJobControlPage({ params }: PageParams) {
  ...
  if (!session?.role) {
    redirect(`/v2/login?next=${encodeURIComponent(`/v2/jobs/${jobId}/job-control`)}`);
  }
  ...
  if (jobResult.kind === "not_found" || jobResult.kind === "forbidden") {
    return (
      <AdminShell title="Job control" breadcrumb={<BackToJobs />}>
  ...
  if (jobResult.kind === "error") {
    return (
      <AdminShell title="Job control" breadcrumb={<BackToJob jobId={jobId} />}>
        <div className="mx-auto max-w-3xl space-y-4">
