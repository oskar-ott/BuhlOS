...
describe("singleJobTarget", () => {
  it("falls back to the jobs index when zero or multiple jobs are affected", () => {
    ...
    expect(singleJobTarget([job({ id: "a" }), job({ id: "b" })], "snags")).toEqual({
      href: "/v2/jobs",
      cta: "Open jobs",
    });
  });

  it("keeps normal single-job evidence and snags links readable", () => {
    expect(singleJobTarget([job({ id: "birdwood-iv3232" })], "evidence")).toEqual({
      href: "/v2/jobs/birdwood-iv3232/evidence",
      cta: "Open evidence",
    });
    expect(singleJobTarget([job({ id: "birdwood-iv3232" })], "snags")).toEqual({
      href: "/v2/jobs/birdwood-iv3232/snags",
      cta: "Open snags",
    });
  });

  it("encodes unsafe job id characters in single-job evidence and snags links", () => {
    const oddJob = job({ id: "job/with?bad#chars" });
    const evidence = singleJobTarget([oddJob], "evidence");
    const snags = singleJobTarget([oddJob], "snags");

    expect(evidence.href).toBe("/v2/jobs/job%2Fwith%3Fbad%23chars/evidence");
    expect(snags.href).toBe("/v2/jobs/job%2Fwith%3Fbad%23chars/snags");
    expect(isSafeActionHref(evidence.href)).toBe(true);
    expect(isSafeActionHref(snags.href)).toBe(true);
    expect(evidence.href).not.toContain("job/with?bad#chars");
    expect(snags.href).not.toContain("job/with?bad#chars");
  });
});
